...
impl Into<Value> for GateError {
    fn into(self) -> Value {
        match self {
            GateError::AssetNotFound => Value::Empty,
            GateError::AuthError { code, msg } => Value::Map({
                let mut map = HashMap::new();
                map.insert("code".to_string(), Value::Int(code));
                map.insert("msg".to_string(), Value::String(msg));
                map
            }),
            GateError::OrderError { code, msg } => Value::Map({
                let mut map = HashMap::new();
                map.insert("code".to_string(), Value::Int(code));
                map.insert("msg".to_string(), Value::String(msg));
                map
...
impl GateBuilder {
    ...
    pub fn public_url<E: Into<Endpoint>>(self, endpoint: E) -> Self {
        match endpoint.into() {
            Endpoint::Linear => Self {
                public_url: "wss://fx-ws.gateio.ws/v4/ws/usdt".to_string(),
                ..self
            },
            Endpoint::Testnet => Self {
                public_url: "wss://fx-ws-testnet.gateio.ws/v4/ws/usdt".to_string(),
                ..self
            },
    ...
    pub fn private_url<E: Into<Endpoint>>(self, endpoint: E) -> Self {
        match endpoint.into() {
            Endpoint::Linear => Self {
                private_url: "wss://fx-ws.gateio.ws/v4/ws/usdt".to_string(),
                ..self
            },
            Endpoint::Testnet => Self {
                private_url: "wss://fx-ws-testnet.gateio.ws/v4/ws/usdt".to_string(),
                ..self
            },
    ...
    pub fn trade_url<E: Into<Endpoint>>(self, endpoint: E) -> Self {
        match endpoint.into() {
            Endpoint::Linear => Self {
                trade_url: "wss://fx-ws.gateio.ws/v4/ws/usdt".to_string(),
                ..self
            },
            Endpoint::Testnet => Self {
                trade_url: "wss://fx-ws-testnet.gateio.ws/v4/ws/usdt".to_string(),
                ..self
            },
    ...
    pub fn rest_url<E: Into<Endpoint>>(self, endpoint: E) -> Self {
        match endpoint.into() {
            ...
            Endpoint::Testnet => Self {
                rest_url: "https://fx-api-testnet.gateio.ws/api/v4".to_string(),
                ..self
            },
    ...
    pub fn subscribe_multiple_depth(mut self) -> Self {
        ...
        self.topics.insert("futures.book_ticker".to_string());
        self.topics.insert("futures.order_book".to_string());
        self.topics.insert("futures.order_book_update".to_string());
        self
    }

    /// Builds [`Gate`] connector.
    pub fn build(self) -> Result<Gate, BuildError> {
        ...
        Ok(Gate {
            ...
            client: GateClient::new(&self.rest_url, &self.api_key, &self.secret),
        })
    }
}
